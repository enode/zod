...
class ZodDiscriminatedUnion extends ZodType {
    ...
    _enforceParentDiscriminator(discriminator) {
        const valueSet = new Set();
        for (const type of this._def.options) {
            if (type instanceof ZodObject) {
                const discriminatorValues = getDiscriminator(type.shape[discriminator]);
                if (!discriminatorValues.length) {
                    throw new Error(`A discriminator value for key \`${discriminator}\` ` +
                ...
                if (values.length < 1) {
                    throw new Error(`No value for key \`${discriminator}\` was found for DiscriminatedUnion with discriminator \`${type.discriminator}\``);
                }
    ...
    static create(discriminator, options, params) {
        ...
        const homologueAssertion = (option) => option instanceof ZodDiscriminatedUnion &&
        ...
        const addUniqueDiscriminatorValues = (values, type) => {
